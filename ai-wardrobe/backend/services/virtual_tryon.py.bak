...
def _remove_garment_background(img: Image.Image) -> Image.Image:
    """
    Strips white, light-gray, or uniform background colors from the garment image,
    creating a clean transparent alpha channel for seamless blending.
    """
    if img.mode != "RGBA":
    ...
    is_light = avg_r > 180 and avg_g > 180 and avg_b > 180
    is_dark = avg_r < 70 and avg_g < 70 and avg_b < 70
    ...
    for item in datas:
        ...
        dist_bg = ((r - avg_r)**2 + (g - avg_g)**2 + (b - avg_b)**2)**0.5
        dist_white = ((r - 255)**2 + (g - 255)**2 + (b - 255)**2)**0.5
        
        if is_light:
            if dist_white < 25 or dist_bg < 30 or (r > 240 and g > 240 and b > 240):
                new_data.append((r, g, b, 0))
            elif dist_bg < 60:
                factor = (dist_bg - 25) / 35.0
                new_data.append((r, g, b, int(a * max(0.0, min(1.0, factor)))))
            else:
                new_data.append(item)
        elif is_dark:
            if dist_bg < 30 or (r < 25 and g < 25 and b < 25):
                new_data.append((r, g, b, 0))
            elif dist_bg < 60:
                factor = (dist_bg - 30) / 30.0
                new_data.append((r, g, b, int(a * max(0.0, min(1.0, factor)))))
            else:
                new_data.append(item)
        else:
            if dist_white < 25 or (r > 242 and g > 242 and b > 242):
                new_data.append((r, g, b, 0))
            elif r > 230 and g > 230 and b > 230:
                factor = (r + g + b - 690) / 75.0
    ...
    # Smooth boundaries using a Gaussian-blurred alpha channel mask
    alpha = img.split()[3]
...
def _compose_try_on(body: Image.Image, clothing: Image.Image) -> Image.Image:
    ...
    # Estimate human background color from corners
    corner_samples = []
    for x in (0, 1, 2, w-1, w-2, w-3):
        for y in (0, 1, 2, h-1, h-2, h-3):
            if 0 <= x < w and 0 <= y < h:
                corner_samples.append(body.getpixel((x, y)))
    if not corner_samples:
        corner_samples = [(255, 255, 255, 255)]
    ...
    # Create silhouette mask (255 for body, 0 for background)
    silhouette = Image.new("L", (w, h), 0)
    sil_data = []
    for r, g, b, a in body.getdata():
        if a == 0:
            sil_data.append(0)
            continue
        dist_bg = ((r - avg_r)**2 + (g - avg_g)**2 + (b - avg_b)**2)**0.5
        if dist_bg > 30 and (r < 242 or g < 242 or b < 242):
            sil_data.append(255)
        else:
            sil_data.append(0)
    silhouette.putdata(sil_data)
    ...
    if found_neck:
        for y in range(neck_y, min(neck_y + 120, h)):
            if row_widths[y] >= min_width * 1.35:
                shoulders_y = y
                shoulder_width = row_widths[y]
                break
    else:
        neck_y = int(h * 0.23)
        neck_x = w // 2
        shoulders_y = int(h * 0.29)
        shoulder_width = int(w * 0.62)

    print(f"STAGE: Torso analysis: Neck center=({neck_x}, {neck_y}), Shoulders Y={shoulders_y}, Shoulder Width={shoulder_width}")

    # Scale garment proportionally (maintain aspect ratio) to match shoulder width
    gc_w, gc_h = garment_cropped.size
    
    # 8. Avoid oversized overlays: target garment width is 82% of shoulder width
    target_g_w = int(shoulder_width * 0.82)
    scale_factor = target_g_w / gc_w
    target_g_h = int(gc_h * scale_factor)

    print(f"STAGE: Proportional scaling: {gc_w}x{gc_h} scaled to {target_g_w}x{target_g_h} (Preserving Aspect Ratio)")
    aligned_g = garment_cropped.resize((target_g_w, target_g_h), Image.Resampling.LANCZOS)

    # Position garment neck/collar over the detected neckline constriction
    x_pos = neck_x - (target_g_w // 2)
    
    # Neckline collar alignment: center neckline with shoulders y-offset safely
    y_pos = int(neck_y - (target_g_h * 0.04))
    # Constrain positioning so it doesn't shift too high above neck
    y_pos = max(y_pos, head_top + 20)
...
def get_hf_token() -> str:
    """
    Dynamically loads HF_TOKEN from .env file to prevent having to restart the backend server
    when the user modifies the .env file.
    """
    try:
        ...
        # Resolve backend directory relative to this service file
        ...
        if token and not token.startswith("your_"):
            ...
            print("HF TOKEN FOUND BUT IT IS UNSET OR EMPTY")
        return token
    except Exception as e:
        print(f"Error loading HF_TOKEN from dotenv: {e}")
        from config import HF_TOKEN
        token = HF_TOKEN or ""
        if token and not token.startswith("your_"):
            print("HF TOKEN FOUND")
...
def generate_try_on(body_image_url: str = None, clothing_image_url: str = None,
    ...
    try:
        ...
        if result_image is not None:
            ...
            print("STAGE ROUTING: Advanced VTON returned None.")
    except Exception as e:
        print("STAGE ROUTING: Advanced VTON inference exception:")
        traceback.print_exc()
        import logging
        logger = logging.getLogger(__name__)
        logger.warning(f"Advanced VTON inference failed: {e}, trying local model...")
    ...
    try:
        ...
        if result_image is not None:
            ...
            print("STAGE ROUTING: CatVTON returned None.")
    except Exception as e:
        print("STAGE ROUTING: CatVTON inference exception:")
        traceback.print_exc()
        import logging
        logger = logging.getLogger(__name__)
        logger.warning(f"CatVTON inference failed: {e}, trying local model...")
