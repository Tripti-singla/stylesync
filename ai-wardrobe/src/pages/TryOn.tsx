...
const TryOnPage = () => {
  ...
  const canGenerate = useMemo(() => Boolean(bodyFile && clothingFile), [bodyFile, clothingFile]);
  ...
  const generateTryOn = async () => {
    ...
    try {
      ...
      const selectedClothingFile = clothingFile;

      const formData = new FormData();
      formData.append("user_id", USER_ID);
      formData.append("occasion", occasion);
      formData.append("body_image", selectedBodyFile);
      formData.append("clothing_image", selectedClothingFile);
      ...
      setResultModel(result.model || "fallback");
      ...
      if (result.clothing_analysis) {
        setStatusMessage(
          `Generated with ${result.model} mode. Detected ${result.clothing_analysis.category || "item"} in ${result.clothing_analysis.style || "a clean silhouette"}.${recommendationMessage}`
        );
      } else {
        setStatusMessage(`Generated with ${result.model} mode.${recommendationMessage}`);
      }
  ...
  const previewSummary = [
    { label: "Body photo", value: bodyFile ? bodyFile.name : "No file selected" },
    { label: "Garment photo", value: clothingFile ? clothingFile.name : "No file selected" },
      ...
      <div className="container py-8 md:py-10">
        ...
        <div className="mt-8 grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
              <motion.label
                ...
                className="group cursor-pointer overflow-hidden rounded-[1.75rem] border border-border bg-card p-5 transition hover:border-primary/40 hover:shadow-product-hover"
              >
                ...
                <div className="mt-5 overflow-hidden rounded-2xl border border-dashed border-border bg-background/80">
                  {bodyPreview ? (
                    <img src={bodyPreview} alt="Body preview" className="h-56 w-full object-cover" />
                  ) : (
              ...
              >
                ...
                <div className="mt-5 overflow-hidden rounded-2xl border border-dashed border-border bg-background/80">
                  {clothingPreview ? (
                    <img src={clothingPreview} alt="Clothing preview" className="h-56 w-full object-cover" />
                  ) : (
                    <div className="flex h-56 flex-col items-center justify-center px-6 text-center text-sm text-muted-foreground">
                      <div className="rounded-full bg-muted p-3 text-primary">
            ...
            <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
              <motion.div
                ...
                className="rounded-[1.75rem] border border-border bg-card p-5 shadow-product"
              >
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <div className="overflow-hidden rounded-2xl border border-border bg-background">
                    {bodyPreview ? (
                      <img src={bodyPreview} alt="Selected body preview" className="h-56 w-full object-cover" />
                    ) : (
                  ...
                  <div className="overflow-hidden rounded-2xl border border-border bg-background">
                    {clothingPreview ? (
                      <img src={clothingPreview} alt="Selected clothing preview" className="h-56 w-full object-cover" />
                    ) : (
              ...
              >
                ...
                <div className="mt-4 space-y-3">
                  {previewSummary.map((entry) => (
                    <div key={entry.label} className="rounded-2xl border border-border bg-background px-4 py-3">
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 rounded-2xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground transition hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                    ...
                    <button
                      ...
                      disabled={!clothingFile || isMatching}
                      className="inline-flex items-center justify-center gap-2 rounded-2xl border border-border bg-background px-5 py-3 text-sm font-semibold text-foreground transition hover:border-primary/40 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                    ...
                    <button
                      ...
                      disabled={isTraining}
                      className="inline-flex items-center justify-center gap-2 rounded-2xl border border-primary/20 bg-primary/10 px-5 py-3 text-sm font-semibold text-primary transition hover:bg-primary/15 disabled:cursor-not-allowed disabled:opacity-60"
                    >
          ...
          <motion.aside
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.15 }}
            className="h-fit rounded-[1.75rem] border border-border bg-card p-5 shadow-product lg:sticky lg:top-6"
          >
            <div className="flex items-center justify-between gap-4">
              <div>
                <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">
                  <Image className="w-3.5 h-3.5" /> Result
                </div>
                <h2 className="mt-3 text-2xl font-display font-bold text-foreground">Try-on output</h2>
              </div>
              <div className="rounded-2xl border border-border bg-background px-3 py-2 text-right">
                <div className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Model</div>
                <div className="text-sm font-semibold text-foreground">{resultModel || "Waiting"}</div>
              </div>
            </div>

            <div className="mt-4 overflow-hidden rounded-[1.5rem] border border-border bg-background">
              {tryonResult ? (
                <img src={tryonResult} alt="Try-on result" className="h-[520px] w-full object-cover" />
              ) : (
                <div className="flex h-[520px] flex-col items-center justify-center px-8 text-center">
                  <div className="rounded-full bg-primary/10 p-4 text-primary">
                    <Sparkles className="w-6 h-6" />
                  </div>
                  <p className="mt-4 text-lg font-semibold text-foreground">Your generated look lands here</p>
                  <p className="mt-2 text-sm text-muted-foreground">
                    Upload a body photo and garment image, then generate to see the result in the panel.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="mt-4 rounded-2xl border border-border bg-background p-4">
              ...
              <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                <p>• Generates a try-on image from the uploaded body and clothing photos.</p>
                <p>• Analyzes the garment and fetches wardrobe/external matches.</p>
                <p>• Saves samples so you can keep improving the model later.</p>
              </div>
            </div>
          </motion.aside>
        </div>
        ...
        {(wardrobeMatches.length > 0 || productMatches.length > 0) && (
          ...
          >
            <div className="rounded-[1.75rem] border border-border bg-card p-5 shadow-product">
